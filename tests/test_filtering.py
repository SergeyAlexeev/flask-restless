...
class TestFiltering(ManagerTestBase):
    ...
    def test_dates(self):
        """Test for date parsing in filter objects."""
        person1 = self.Person(id=1, birthday=datetime.date(1969, 7, 20))
        person2 = self.Person(id=2, birthday=datetime.date(1900, 1, 2))
        self.session.add_all([person1, person2])
        self.session.commit()

        def search(datestring):
            filters = [dict(name='birthday', op='eq', val=datestring)]
            return self.search('/api/person', filters)

        response = search('1900-01-02')
        document = loads(response.data)
        people = document['data']
        assert len(people) == 1
        assert people[0]['id'] == 1

        response = search('2nd Jan 1900')
        document = loads(response.data)
        people = document['data']
        assert len(people) == 1
        assert people[0]['id'] == 1
    ...
    def test_datetime_to_date(self):
        """Tests that a datetime gets truncated to a date if the model has a
        date field.
